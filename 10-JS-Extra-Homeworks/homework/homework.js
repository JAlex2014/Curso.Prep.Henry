...
function capicua(numero){
  ...
  //Escribe tu código aquí
  var digitosiguales = true;
  var cadena = String(numero);
  for(var i=0; i < (cadena.length)/2; i++){
    digitosiguales = digitosiguales && (cadena[i] === cadena[cadena.length-(i+1)]);
  }
  if (digitosiguales){
    return "Es capicua";
  }
...
function deleteAbc(cadena){
  ...
  //Escribe tu código aquí
function eliminaCaracter(string, caracter){
  var nuevastring = "";
  for(var i= 0; i < string.length; i++){
    if(string.charAt(i) !== caracter){
      nuevastring = nuevastring + string.charAt(i);
    }
  }
  return nuevastring;
}
cadena = eliminaCaracter(cadena, "a");
cadena = eliminaCaracter(cadena, "b");
cadena = eliminaCaracter(cadena, "c");
return cadena;
}
